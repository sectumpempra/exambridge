...
| 模块 | 作用 | 发布边界 |
| --- | --- | --- |
| 课程中心 | 按阶段、考试局、资格、科目和学科类别检索课程 | 版本化目录当前包含 714 门课程 |
| 分数线 | 查询不同考试局与考季的等级门槛 | 已核验和待核验记录明确区分 |
| 成绩统计 | 查看等级分布和群体表现 | WJEC/Eduqas 当前仅开放此类数据 |
| Paper 中心 | 检索试卷并连接课程与考纲 | 未覆盖课程会显示不可用原因 |
...
- Next/vinext 构建并部署至 Cloudflare Sites
...
1. 在 `agent/release-hardening` 或功能分支完成变更。
2. 通过冻结安装、数据审计、安全审计、覆盖率、E2E 和生产构建门禁。
3. GitHub 源码、Sites 归档与部署版本对应同一提交 SHA。
4. 先发布为私有灰度版本，验证全路由、导出、离线升级和核心计算。
5. 仅在灰度通过并获得明确确认后调整公开访问策略；保留前一版本作为回滚点。
