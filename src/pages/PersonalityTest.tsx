...
export default function PersonalityTest() {
  ...
  return (
    <div key={queryKey} style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "linear-gradient(180deg, #F0EDE8, #F5F2EE)" }}>
      <Header title={isTeacher ? "A-Level 教师人格诊断" : "A-Level 学科人格诊断"} />
          ...
          {page === "landing" && (
            <div style={{ ...cardStyle, textAlign: "center", padding: "48px 32px" }}>
              <div style={{ fontSize: 56, marginBottom: 16, animation: "float 3s ease-in-out infinite" }}>
                {isTeacher ? <GraduationCap size={48} style={{ color: "#675A4D" }} /> : <Sparkles size={48} style={{ color: "#675A4D" }} />}
              </div>
              <h1 style={{ fontSize: 24, fontWeight: 700, color: "#3D3832", margin: "0 0 8px" }}>
                {isTeacher ? "A-Level 教师人格诊断" : "A-Level 学科人格诊断"}
              </h1>
              <p style={{ fontSize: 15, color: "#625C54", margin: "0 0 32px" }}>
                {isTeacher
                  ? "测测你是什么样的 A-Level 教师？"
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 24 }}>
                {["20 道趣味题目", "4 维人格分析", "16 种独特人格"].map(t => (
                  <span key={t} style={{ padding: "6px 14px", borderRadius: 20, background: "rgba(143,127,110,0.08)", color: "#675A4D", fontSize: 13, fontWeight: 500 }}>
                    {t}
                  </span>
                ))}
              </div>
              ...
              <button onClick={startQuiz}
                style={{
                  ...
                  background: "linear-gradient(135deg, #675A4D, #A69888)", color: "#FFF",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ ...cardStyle, padding: "16px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: "#6E675E", fontWeight: 500 }}>第 {currentQ + 1} / {questions.length} 题</span>
                  <span style={{ fontSize: 12, color: "#6E675E" }}>{Math.round(progress)}%</span>
                </div>
                <div style={{ height: 6, background: "#E8E4DE", borderRadius: 3, overflow: "hidden" }}>
                  <div style={{ width: `${progress}%`, height: "100%", background: "linear-gradient(90deg, #526B7E, #775E55)", borderRadius: 3, transition: "width 0.4s ease" }} />
                </div>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {q.options.map((opt, i) => (
                    <button key={i} onClick={() => handleAnswer(i)}
                      ...
                      onMouseEnter={e => { e.currentTarget.style.borderColor = "#675A4D"; e.currentTarget.style.background = "rgba(143,127,110,0.04)"; }}
                      ...
                      <span style={{
                        ...
                        color: "#675A4D", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                      }}>{opt.label}</span>
                      <span>{opt.text}</span>
                    </button>
                  ))}
          ...
          {page === "loading" && (
            <div style={{ ...cardStyle, textAlign: "center", padding: "80px 32px" }}>
              <div style={{
                ...
                border: "3px solid #E8E4DE", borderTopColor: "#675A4D",
                margin: "0 auto 24px", animation: "spin 1s linear infinite",
              }} />
              <p style={{ fontSize: 16, color: "#625C54", margin: 0 }}>正在解析你的人格...</p>
          ...
          {page === "result" && result && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: 14, color: "#625C54", fontStyle: "italic", margin: 0 }}>{result.tagline}</p>
              </div>
              ...
              <div style={cardStyle}>
                ...
                <div style={{ marginTop: 14, padding: "12px 16px", background: "rgba(143,127,110,0.06)", borderRadius: 10, borderLeft: "3px solid #675A4D" }}>
              ...
              <div style={cardStyle}>
                <h4 style={{ fontSize: 15, fontWeight: 600, color: "#3D3832", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 6 }}>
                  <Heart size={16} style={{ color: "#775E55" }} /> 软肋
                </h4>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              ...
              <div style={cardStyle}>
                ...
                <p style={{ fontSize: 13, color: "#625C54", margin: 0 }}>{result.partners}</p>
              </div>

              {/* Actions */}
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={handleShare}
                  style={{ flex: 1, padding: "12px", borderRadius: 10, background: "linear-gradient(135deg, #675A4D, #A69888)", color: "#FFF", fontSize: 14, fontWeight: 600, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                  <Clipboard size={16} /> {copied ? "已复制" : "分享结果"}
                </button>
                <button onClick={handleRestart}
                  style={{ flex: 1, padding: "12px", borderRadius: 10, background: "#FFF", color: "#675A4D", fontSize: 14, fontWeight: 600, border: "1px solid #D9D4CE", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
...
function DimensionBars({ dimensions }: { dimensions: Record<string, number> }) {
  ...
  const bars = [
    { key: "EI", label: "外向 E", rightLabel: "内向 I", leftPct: pct.EI.left, leftColor: "#526B7E", rightColor: "#775E55" },
    { key: "SN", label: "实感 S", rightLabel: "直觉 N", leftPct: pct.SN.left, leftColor: "#506D58", rightColor: "#655A70" },
    { key: "TF", label: "理性 T", rightLabel: "感性 F", leftPct: pct.TF.left, leftColor: "#675A4D", rightColor: "#C17B5F" },
    { key: "JP", label: "计划 J", rightLabel: "随性 P", leftPct: pct.JP.left, leftColor: "#6E5C40", rightColor: "#A0A8B0" },
  ];
  ...
  return (
    <div style={{ padding: 20, borderRadius: 16, background: "linear-gradient(135deg, rgba(255,255,255,0.95), rgba(250,248,245,0.9))", boxShadow: "0 4px 20px rgba(61,56,50,0.06)", border: "1px solid rgba(233,229,222,0.8)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {bars.map(b => (
          <div key={b.key}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#625C54", marginBottom: 4 }}>
              <span>{b.label}</span>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#6E675E", marginTop: 2 }}>
