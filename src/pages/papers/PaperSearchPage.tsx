...
export default function PaperSearchPage() {
  ...
  useEffect(() => {
    if (!entry || entry.capabilities.papers.status === "unavailable" || appliedCourse.current === entry.qualificationId) return;
    const compatible = ALL_PAPERS.find((paper) => paper.board === entry.boardName && paper.subjectCode === entry.subjectCode);
    if (!compatible) return;
    appliedCourse.current = entry.qualificationId;
    queueMicrotask(() => {
      setBoardFilter(entry.boardName);
      setQualFilter(compatible.qualification);
      setSearch(entry.subjectCode);
    });
  }, [entry]);
        ...
        <section style={{ padding: "40px 16px 24px", textAlign: "center" }}>
          <div style={{ maxWidth: 800, margin: "0 auto" }}>
            <h1 style={{ fontSize: "clamp(24px, 4vw, 32px)", fontWeight: 700, color: "#3D3832", margin: 0, letterSpacing: "0.02em" }}>
              <span style={{ color: "#675A4D" }}>按 Paper</span> 查询考试信息
            </h1>
            <p style={{ fontSize: 14, color: "#6E675E", marginTop: 10, lineHeight: 1.7 }}>
        ...
        <section style={{ padding: "0 16px 48px" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            {groupedBySubject.size === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px", color: "#6E675E" }}>
                ...
                <div key={subjectKey} style={{ marginBottom: 24 }}>
                  ...
                  <div style={{
                    ...
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: "#3D3832", display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ color: "#675A4D" }}>{papers[0].board}</span>
                      <span style={{ color: "#6E675E", fontWeight: 400 }}>/</span>
                      {papers[0].subjectName}
                      <span style={{ fontSize: 12, color: "#6E675E", fontWeight: 400, background: "rgba(166,152,136,0.12)", padding: "2px 8px", borderRadius: 6 }}>
                        {papers[0].subjectCode}
                      </span>
                      <span style={{ fontSize: 12, color: "#6E675E", fontWeight: 400, marginLeft: 4 }}>
                    ...
                    {papers.map((paper, idx) => (
                      ...
                      >
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", fontSize: 12, color: "#625C54" }}>
                        ...
                        <ChevronRight size={18} style={{ color: "#716A61", flexShrink: 0, marginLeft: 8 }} />
                      </Link>
                    ))}
                  </div>
                </div>
