...
import { labelExpression } from './displayUtils';
import {
  create, parseDependencies, addDependencies, subtractDependencies, multiplyDependencies,
  divideDependencies, powDependencies, modDependencies, unaryMinusDependencies, unaryPlusDependencies,
  factorialDependencies, sinDependencies, cosDependencies, tanDependencies, asinDependencies,
  acosDependencies, atanDependencies, atan2Dependencies, sinhDependencies, coshDependencies,
  tanhDependencies, asinhDependencies, acoshDependencies, atanhDependencies, secDependencies,
  cscDependencies, cotDependencies, logDependencies, log10Dependencies, log2Dependencies,
  sqrtDependencies, cbrtDependencies, expDependencies, absDependencies, roundDependencies,
  floorDependencies, ceilDependencies, fixDependencies, maxDependencies, minDependencies,
  signDependencies,
} from 'mathjs/number';

const math = create({
  parseDependencies, addDependencies, subtractDependencies, multiplyDependencies,
  divideDependencies, powDependencies, modDependencies, unaryMinusDependencies, unaryPlusDependencies,
  factorialDependencies, sinDependencies, cosDependencies, tanDependencies, asinDependencies,
  acosDependencies, atanDependencies, atan2Dependencies, sinhDependencies, coshDependencies,
  tanhDependencies, asinhDependencies, acoshDependencies, atanhDependencies, secDependencies,
...
const MATH_BUILTINS = new Set([
  ...
  'sec', 'csc', 'cot',
  'log', 'log10', 'log2', 'sqrt', 'cbrt', 'exp', 'abs', 'pow',
  'round', 'floor', 'ceil', 'fix', 'mod', 'factorial',
  'max', 'min', 'sign',
  'e', 'pi', 'x', 'theta', 't',
]);
...
const SAFE_NODE_TYPES = new Set(['OperatorNode', 'ConstantNode', 'SymbolNode', 'ParenthesisNode', 'FunctionNode']);

function parseSafeExpression(expression: string) {
  if (!expression || expression.length > GRAPH_LIMITS.expressionLength) return null;
  try {
    ...
    node.traverse((child) => {
      nodeCount += 1;
      const candidate = child as unknown as { type?: string; isSymbolNode?: boolean; isFunctionNode?: boolean; name?: string; fn?: { name?: string } };
      if (!candidate.type || !SAFE_NODE_TYPES.has(candidate.type) || nodeCount > GRAPH_LIMITS.astNodes) safe = false;
      if (candidate.isFunctionNode) {
        const functionName = candidate.fn?.name?.toLowerCase();
        if (!functionName || !MATH_BUILTINS.has(functionName)) safe = false;
      }
      if (candidate.isSymbolNode && candidate.name) {
        const name = candidate.name.toLowerCase();
        if (!MATH_BUILTINS.has(name)) {
          if (!/^(?:[a-z]|p\d{1,2})$/i.test(candidate.name)) safe = false;
          params.add(candidate.name);
        }
      }
    });
    return safe && params.size <= GRAPH_LIMITS.parameters ? node : null;
  } catch {
    return null;
  }
}
...
/**
 * Keep the expression shown to teachers in conventional implicit form.
 * Evaluation still goes through preprocessExpression, so this is display-safe
 * as well as slider-safe: changing a parameter never rewrites the formula.
 */
export function formatImplicitMultiplication(expression: string): string {
  const protectedStarts = [...NAMES_TO_PROTECT, 'e'].sort((a, b) => b.length - a.length);
  let result = '';
  for (let index = 0; index < expression.length; index++) {
    ...
    const rest = expression.slice(index + 1).toLowerCase();
    const needsBoundaryAfter = protectedStarts.some(name =>
      rest.startsWith(name) && (rest.length === name.length || !/[a-z]/.test(rest[name.length] ?? ''))
    );
    const lowerResult = result.toLowerCase();
    const needsBoundaryBefore = protectedStarts.some(name => lowerResult.endsWith(name));
    if ((needsBoundaryAfter || needsBoundaryBefore) && result && !result.endsWith(' ')) result += ' ';
  }
  return result;
}
...
export function extractParams(expression: string): string[] {
  if (!expression || !expression.trim() || expression.length > GRAPH_LIMITS.expressionLength) return [];
  const mode = detectMode(expression);
  const body = getExpressionBody(expression, mode);
  const preprocessed = preprocessExpression(body);
  try {
    const node = parseSafeExpression(preprocessed);
    if (!node) return [];
...
export function convertNumbersToParams(expression: string): { expression: string; params: Record<string, number> } | null {
  ...
  result = formatImplicitMultiplication(result);

  // Step 4: Verify the new expression compiles
  try {
    math.parse(preprocessExpression(result)).compile();
  } catch { return null; }
...
export function compileExpression(expression: string, mode?: FunctionMode): EvalFunction | null {
  if (!expression || !expression.trim() || expression.length > GRAPH_LIMITS.expressionLength) return null;
  const detectedMode = mode || detectMode(expression);
  const body = getExpressionBody(expression, detectedMode);
  const preprocessed = preprocessExpression(body);
  try { return parseSafeExpression(preprocessed)?.compile() ?? null; } catch { return null; }
}

// ==== DOMAIN PARSING ====
function parseDomainValue(val: string): number | null {
  if (!val || !val.trim() || val.length > 64) return null;
  try {
    const node = parseSafeExpression(val);
    if (!node) return null;
    const symbols = new Set<string>();
    node.traverse((child) => {
      const symbol = child as unknown as { isSymbolNode?: boolean; name?: string };
      if (symbol.isSymbolNode && symbol.name) symbols.add(symbol.name.toLowerCase());
    });
    if ([...symbols].some((name) => name !== 'pi' && name !== 'e')) return null;
    const result = node.compile().evaluate({ pi: Math.PI, e: Math.E });
...
function drawHorizontalAsymptote(ctx: CanvasRenderingContext2D, y: number, color: string, w: number) {
  ctx.save();
  ctx.strokeStyle = color;
  ctx.globalAlpha = 0.85;
  ctx.setLineDash([10, 5]);
  ...
  if (entry.showAsymptotes) {
    ...
    if (hasExpAsymptote || hasRationalAsymptote) {
      ...
      if (hCanvas > 0 && hCanvas < h) {
        drawHorizontalAsymptote(ctx, hCanvas, entry.color, w);
      }
    }
    ...
    if (exprLower.includes('tanh(') && p.a !== undefined) {
      ...
      if (upperY > 0 && upperY < h) {
        drawHorizontalAsymptote(ctx, upperY, entry.color, w);
      }
      const lowerY = cy - (dVal - aVal) * up;
      if (lowerY > 0 && lowerY < h) {
        drawHorizontalAsymptote(ctx, lowerY, entry.color, w);
      }
    }
...
function drawFuncLabel(ctx: CanvasRenderingContext2D, entry: FunctionEntry, w: number, h: number, view: ViewState, li: number, placedLabels: { top: number; bottom: number }[]) {
  ...
  const label = labelExpression(entry.expression, entry.mode, entry.params);
