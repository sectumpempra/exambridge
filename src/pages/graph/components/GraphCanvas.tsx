...
export default function GraphCanvas({ functions, intersections = [], onClearIntersections }: GraphCanvasProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="graph-canvas-surface relative h-full w-full select-none overflow-hidden"
      tabIndex={0}
    >
      <canvas
        role="img"
        aria-label="函数图像画布，可使用鼠标或键盘缩放和平移"
        tabIndex={0}
        ref={canvasRef}
      ...
      {/* Zoom controls */}
      <div className="absolute left-4 top-4 z-20 flex flex-col gap-1 overflow-hidden rounded-xl border border-[#ccd9e7] bg-white/95 p-1 shadow-lg shadow-slate-200/70 backdrop-blur">
        <button onClick={() => zoom(1.25)} className="rounded-lg bg-[#eaf3fb] p-2 transition-colors hover:bg-[#d8e9f8]" title="放大 (+)">
          <ZoomIn className="h-4 w-4 text-[#315f86]" />
        </button>
        <button onClick={() => zoom(0.8)} className="rounded-lg bg-[#edf8f4] p-2 transition-colors hover:bg-[#d9efe7]" title="缩小 (-)">
          <ZoomOut className="h-4 w-4 text-[#39725d]" />
        </button>
        <button onClick={resetView} className="rounded-lg bg-[#fff4e5] p-2 transition-colors hover:bg-[#fce5c3]" title="重置视图 (0)">
          <Maximize className="h-4 w-4 text-[#9a641c]" />
        </button>
        <button
          onClick={cycleAxisMode}
          className={`mono-num rounded-lg p-2 text-xs font-bold transition-colors ${
            axisMode !== 'number' ? 'bg-[#6650a4] text-white' : 'bg-[#f3effc] text-[#6650a4] hover:bg-[#e7def7]'
          }`}
          title="切换横坐标刻度 (x / π / °)"
        >
        ...
        <button
          ...
          className={`rounded-lg p-2 transition-colors ${
            showLegend ? 'bg-[#334155] text-white' : 'bg-[#eef2f7] text-[#334155] hover:bg-[#dfe6ee]'
          }`}
          title="切换图例 (L)"
        >
      ...
      {showLegend && legendFuncs.length > 0 && (
        <div className="absolute right-4 top-4 z-20 max-w-[220px] rounded-xl border border-[#ccd9e7] bg-white/95 p-3 shadow-lg shadow-slate-200/60 backdrop-blur">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#334155]">函数图例</h4>
          <div className="space-y-1.5">
            {legendFuncs.map((f) => (
              <div key={f.id} className="flex items-center gap-2 text-xs">
                <span className="h-2.5 w-2.5 rounded-full flex-shrink-0" style={{ background: f.color }} />
                <span className="mono-num text-[#334155] truncate" title={labelExpression(f.expression, f.mode, f.params)}>
                  {labelExpression(f.expression, f.mode, f.params)}
                </span>
              </div>
            ))}
          </div>
            ...
            <div className="mt-2 pt-2 border-t border-[#e5e5e5]">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[10px] text-[#626262] hover:text-[#333] transition-colors"
                >
                  清除
                </button>
              </div>
      ...
      <div className="absolute bottom-3 left-4 z-10 pointer-events-none">
        <span className="mono-num rounded-full bg-white/85 px-3 py-1 text-[10px] text-[#64748b] shadow-sm">
          +/- 缩放 &nbsp;|&nbsp; 0 重置 &nbsp;|&nbsp; L 图例 &nbsp;|&nbsp; 拖拽平移
        </span>
      </div>
      ...
      {tooltip && (
        <div
          className="tooltip-fade absolute z-10 rounded-lg border border-[#7da1c4] bg-white px-3 py-2 shadow-lg pointer-events-none"
