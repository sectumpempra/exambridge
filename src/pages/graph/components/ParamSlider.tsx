  ...
  return (
    <div className="py-1" tabIndex={0} onKeyDown={handleContainerKeyDown}>
      ...
      <div className="flex items-center gap-2">
        ...
        {label && (
          <span className="text-[10px] text-[#626262] flex-shrink-0 mr-1">{label}</span>
        )}
        <div className="flex-1 min-w-0">
          <input aria-label={`参数 ${name}`} type="range" min={min} max={max} step={step} value={value} onChange={handleSliderChange} className="graph-param-slider" style={{ accentColor }} />
        </div>
        {isEditing ? (
          <input aria-label={`参数 ${name} 数值`} type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)} onBlur={handleEditSubmit} onKeyDown={handleEditKeyDown} autoFocus
          ...
          <button onClick={() => { setEditValue(value.toFixed(2)); setIsEditing(true); }}
            className={`mono-num w-[52px] text-right text-xs text-black hover:bg-[#f0f0f0] px-1 py-0.5 transition-opacity flex-shrink-0 ${isAtDefault ? 'text-[#626262]' : 'text-black font-medium'}`}>
            {displayVal}
          </button>
        )}
        <button onClick={togglePlay} className={`flex-shrink-0 rounded-md p-1 transition-colors ${isPlaying ? 'text-white' : 'bg-[#eef3f8] text-[#5e7188] hover:bg-[#dfeaf4]'}`} style={isPlaying ? { backgroundColor: accentColor } : undefined} title={isPlaying ? '暂停' : '播放'}>
          {isPlaying ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
        </button>
        <button onClick={() => setShowRange(!showRange)} className="flex-shrink-0 rounded-md bg-[#fff5df] p-1 text-[#8b631f] transition-colors hover:bg-[#f9e6b9]" title="调整范围">
      ...
      {showRange && (
        <div className="flex items-center gap-2 mt-1 pl-5 pr-1">
          <span className="text-[10px] text-[#626262]">范围</span>
          <input aria-label={`参数 ${name} 最小值`} type="text" value={rangeMin} onChange={(e) => { setRangeMin(e.target.value); const v = parseFloat(e.target.value); if (!isNaN(v)) onRangeChange?.(name, v, max, step); }}
          ...
          <span className="text-[10px] text-[#626262]">~</span>
          <input aria-label={`参数 ${name} 最大值`} type="text" value={rangeMax} onChange={(e) => { setRangeMax(e.target.value); const v = parseFloat(e.target.value); if (!isNaN(v)) onRangeChange?.(name, min, v, step); }}
            className="w-10 text-[10px] text-center mono-num border-b border-[#ddd] focus:border-black focus:outline-none text-[#6c6c6c] py-0.5" />
          <span className="text-[10px] text-[#626262]">步长</span>
          <input aria-label={`参数 ${name} 步长`} type="text" value={rangeStep} onChange={(e) => { setRangeStep(e.target.value); const v = parseFloat(e.target.value); if (!isNaN(v) && v > 0) onRangeChange?.(name, min, max, v); }}
            className="w-10 text-[10px] text-center mono-num border-b border-[#ddd] focus:border-black focus:outline-none text-[#6c6c6c] py-0.5" />
        </div>
      )}
    </div>
