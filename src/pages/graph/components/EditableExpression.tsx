...
function simplifyForDisplay(expression: string, params: Record<string, number>): string {
  ...
  if (a === 1 && b === 1 && c === 0 && d === 0) {
    ...
    if (normalized === 'asin(bx+c)+d') return 'sin(x)';
    if (normalized === 'acos(bx+c)+d') return 'cos(x)';
    if (normalized === 'atan(bx+c)+d') return 'tan(x)';
  }
  return expression;
}
...
/**
 * Hide additive parameters whose current value is zero without changing the
 * stored expression. The term therefore reappears as soon as its slider moves
 * away from zero. Multiplicative zero parameters are intentionally preserved.
 */
export function simplifyZeroAdditiveParams(
  expression: string,
  params: Record<string, number>
): string {
  ...
  for (const [name, value] of Object.entries(params)) {
    ...
    // ax^2+bx+c, sin(bx+c)+d -> ax^2+bx, sin(bx)
    simplified = simplified.replace(
      new RegExp(`[+-]\\s*\\b${escapedName}\\b(?=\\s*(?:[+\\-)]|$))`, 'g'),
      ''
    );
    // c+bx inside a group, or c-x at the beginning of an expression.
    simplified = simplified.replace(
      new RegExp(`(^|\\()\\s*\\b${escapedName}\\b\\s*(?=[+\\-])`, 'g'),
      '$1'
    );
...
function buildSegments(
  ...
  let substituted = simplifyZeroAdditiveParams(expr, params);
