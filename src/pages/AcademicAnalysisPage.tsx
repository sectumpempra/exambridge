...
export default function AcademicAnalysisPage() {
  ...
  const sourceById = useMemo(() => new Map((manifest?.sources ?? []).map(source => [source.sourceId, source])), [manifest]);
  ...
  return <div className="min-h-screen bg-[linear-gradient(180deg,#f0ede8,#f7f4f0)] text-[#3d3832]">
    ...
    <main className="mx-auto max-w-[1200px] px-4 py-8 sm:px-6">
      ...
      {manifest && tab === "results" && <section className="space-y-5">
        ...
        <div className="rounded-2xl border border-[#ddd6ce] bg-white p-5"><h2 className="m-0 text-lg font-bold">Grade Statistics（辅助背景）</h2><p className="mb-0 mt-2 text-sm leading-6 text-[#716a61]">成绩统计描述群体分布，不是等级分数线，也不用于计算个别学生等级。跨年比较前必须确认地域、考生群体和 cumulative/exclusive 口径一致。</p></div>
        {manifest.statistics.length === 0
          ? <EmptyState title="Grade Statistics 不阻塞规则上线">当前 active 暂无已批准统计行；资格结构和合分规则可以在证据成熟后独立上线。</EmptyState>
          : <div className="overflow-x-auto rounded-2xl border border-[#ddd6ce] bg-white"><table className="min-w-[980px] w-full text-left text-sm"><caption className="sr-only">已批准 Grade Statistics 辅助数据</caption><thead><tr className="bg-[#f2eee8]"><th className="px-4 py-3">资格 / Route</th><th className="px-4 py-3">考季</th><th className="px-4 py-3">地域 / 群体</th><th className="px-4 py-3">范围与口径</th><th className="px-4 py-3">人数</th><th className="px-4 py-3">等级比例</th><th className="px-4 py-3">发布状态 / 来源</th></tr></thead><tbody>{manifest.statistics.map(statistic => {
            const source = statistic.sourceIds.map(sourceId => sourceById.get(sourceId)).find(Boolean);
            return <tr key={statistic.statisticsId} className="border-t border-[#ebe6df] align-top"><td className="px-4 py-3 font-medium">{statistic.awardQualificationId}<span className="block text-xs text-[#81796f]">{statistic.routeId ?? "overall qualification"}</span></td><td className="px-4 py-3">{statistic.year} {statistic.series}</td><td className="px-4 py-3">{statistic.regionScope}<span className="block text-xs text-[#81796f]">{statistic.populationScope}</span></td><td className="px-4 py-3">{statistic.statisticsScope}{statistic.componentCode ? ` · ${statistic.componentCode}` : ""}<span className="block text-xs font-semibold text-[#526b7e]">{statistic.rateKind}</span></td><td className="px-4 py-3">{statistic.candidateCount?.toLocaleString() ?? "未发布"}</td><td className="px-4 py-3">{statistic.gradeOrder.map(grade => `${grade}: ${statistic.gradeRates[grade] ?? "—"}%`).join(" · ")}</td><td className="px-4 py-3">{statistic.publicationStatus}{source && <><a className="mt-1 block text-xs font-semibold text-[#526b7e] underline" href={source.officialUrl} target="_blank" rel="noreferrer">官方来源 ↗</a><span className="block text-[11px] text-[#81796f]">访问 {source.accessedAt}</span></>}</td></tr>;
          })}</tbody></table></div>}
