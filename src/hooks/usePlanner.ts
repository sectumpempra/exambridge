...
export function usePlanner(config: PlannerConfig, pastPapersMap: Record<string, string[]>): PlannerResult {
  return useMemo(() => {
    ...
    // Weekly quota is qualification-based. This keeps Edexcel IAL Mathematics
    // units together instead of treating six unit codes as six subjects.
    const subjectEvents = new Map<string, ExamEvent[]>();
    for (const ev of events) {
      const list = subjectEvents.get(ev.qualificationId) ?? [];
      list.push(ev);
      subjectEvents.set(ev.qualificationId, list);
    }
    ...
    for (let d = 0; d < totalDays; d++) {
      ...
      // Weekly quota per qualification, round-robin within its papers.
      for (const [qualificationId, sEvents] of subjectEvents) {
        if (tasks.length >= maxTasksPerDay) break;
        const scCount = weeklyCounts.get(qualificationId) ?? 0;
        if (scCount >= papersPerWeek) continue;

        // Round-robin pick one paper from this subject's events
        const rrIdx = subjectRrIndex.get(qualificationId) ?? 0;
        let allocated = false;
        for (let attempt = 0; attempt < sEvents.length; attempt++) {
          ...
          subjectRrIndex.set(qualificationId, (rrIdx + attempt + 1) % sEvents.length);
          weeklyCounts.set(qualificationId, scCount + 1);
          allocated = true;
          break;
        }
        if (!allocated) {
          subjectRrIndex.set(qualificationId, 0); // reset if none available
        }
      }
      } // end if (!isRest)
