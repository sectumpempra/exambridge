...
const LABEL_STYLE: React.CSSProperties = { fontSize: 14, fontWeight: 500, color: "#8B8378", whiteSpace: "nowrap" };
...
export default function GradeChart({
  ...
  return (
    <div className="animate-fade-in-up" style={{ animationDelay: "0.2s", opacity: 0 }}>
      <div style={{ height: 1, background: "linear-gradient(90deg, transparent, #D9D4CE, transparent)", marginBottom: 28 }} />

      <h3 style={{ fontSize: 20, fontWeight: 600, color: "#3D3832", margin: 0, letterSpacing: "0.02em" }}>
        {selectedCode ? (
          <><span style={{ color: "#8F7F6E" }}>{titlePrefix}</span>{" — "}{titleCode}{selectedName ? ` (${selectedName})` : ""}</>
        ) : (
          <><span style={{ color: "#8F7F6E" }}>{titlePrefix}</span>{" — 选择科目查看趋势图"}</>
        )}
      </h3>
      ...
      <div style={{ display: "flex", gap: 20, marginTop: 20, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <select value={selectedCode} onChange={e => { setSelectedCode(e.target.value); setSelectedComponent(""); }}
        ...
        {selectedCode && chartData.length > 0 && (
          <span style={{ fontSize: 13, color: "#A8A095" }}>{chartData.length} 个数据点</span>
        )}
      </div>
      ...
      {chartData.length > 0 && (
        <div style={{ marginTop: 24, background: "#FFFFFF", borderRadius: 16, padding: 24, border: "1px solid #E8E4DE", boxShadow: "0 2px 16px rgba(61,56,50,0.06)" }}>
          <ResponsiveContainer width="100%" height={420}>
            <LineChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(166,152,136,0.15)" />
              <XAxis dataKey="session" tick={{ fontSize: 11, fill: "#A8A095" }} angle={-45} textAnchor="end" height={70} axisLine={{ stroke: "#E8E4DE" }} interval={0} />
              <YAxis tick={{ fontSize: 11, fill: "#A8A095" }} label={{ value: "分数", angle: -90, position: "insideLeft", style: { fontSize: 12, fill: "#A8A095" } }} axisLine={{ stroke: "#E8E4DE" }} />
              ...
              <Legend wrapperStyle={{ fontSize: 12, color: "#8B8378", paddingTop: 16 }} />
              <Line type="monotone" dataKey={maxMarkField} stroke="#C4BDB3" strokeWidth={2} strokeDasharray="6 3" dot={false} name="满分" />
              {gradeFields.map(gf => (
