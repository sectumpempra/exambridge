...
interface DataTableProps {
  ...
  urlStateKey?: string;
  onFilteredDataChange?: (rows: Record<string, string | number>[]) => void;
...
export default function DataTable({
  ...
  const defaultPerPage = itemsPerPageOptions[0] ?? 10;
  const parsePerPage = (value: string) => {
    const parsed = Number(value);
    return itemsPerPageOptions.includes(parsed) ? parsed : defaultPerPage;
  };
  const parsePage = (value: string) => {
    const parsed = Number(value);
    return Number.isSafeInteger(parsed) && parsed > 0 ? parsed : 1;
  };
  const validSortColumns = new Set(columns.filter((column) => column.sortable !== false).map((column) => column.key));

  const [itemsPerPage, setItemsPerPage] = useState(() => parsePerPage(getParam("perPage", String(defaultPerPage))));
  const [currentPage, setCurrentPage] = useState(() => parsePage(getParam("page", "1")));
  const [sortColumn, setSortColumn] = useState<string | null>(() => {
    const v = getParam("sort", "");
    return validSortColumns.has(v) ? v : null;
  });
  ...
  const [activeFilters, setActiveFilters] = useState<Record<string, string>>(() => {
    ...
    filterFields.forEach((ff) => {
      const v = urlStateKey ? searchParams.get(`${prefix}filter_${ff.key}`) : null;
      if (v) filters[ff.key] = v;
      else if (initialFilters[ff.key] && getUniqueValues(data, ff.key).includes(initialFilters[ff.key])) filters[ff.key] = initialFilters[ff.key];
    });
    return filters;
  });

  // Browser history can replace the query string without remounting the table.
  useEffect(() => {
    if (!urlStateKey) return;
    let cancelled = false;
    const nextSearch = getParam("search", "");
    const nextPerPage = parsePerPage(getParam("perPage", String(defaultPerPage)));
    const nextPage = parsePage(getParam("page", "1"));
    const nextSort = getParam("sort", "");
    const nextDirection = getParam("sortDir", "asc") === "desc" ? "desc" : "asc";
    const nextFilters: Record<string, string> = {};
    filterFields.forEach((field) => {
      const value = searchParams.get(`${prefix}filter_${field.key}`);
      if (value && getUniqueValues(data, field.key).includes(value)) nextFilters[field.key] = value;
      else if (initialFilters[field.key] && getUniqueValues(data, field.key).includes(initialFilters[field.key])) nextFilters[field.key] = initialFilters[field.key];
    });
    queueMicrotask(() => {
      if (cancelled) return;
      ...
      setSortColumn(validSortColumns.has(nextSort) ? nextSort : null);
      setSortDirection(nextDirection);
      setActiveFilters(nextFilters);
    });
    return () => { cancelled = true; };
    // This effect intentionally tracks the serialized URL as the history source of truth.
  ...
  useEffect(() => {
    ...
    setOrDelete("perPage", String(itemsPerPage), String(defaultPerPage));
  ...
  return (
    <div className="animate-fade-in-up" style={{ animationDelay: "0.3s", opacity: 0 }}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2.5">
        {filterFields.map((ff) => (
          <div key={ff.key} className="flex items-center gap-1.5">
            <label htmlFor={`${idBase}-${ff.key}`} className="whitespace-nowrap text-xs text-[#625C54]">
              {ff.label}
            </label>
            <select
              id={`${idBase}-${ff.key}`}
      ...
      <div className="mt-2.5 flex items-center gap-2">
        <label htmlFor={`${idBase}-per-page`} className="text-xs text-[#6E675E]">每页</label>
        <select
          id={`${idBase}-per-page`}
          value={itemsPerPage}
          onChange={(e) => {
        ...
        <span className="text-xs text-[#6E675E]">条</span>
        <span className="ml-3 text-xs text-[#6E675E]">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2.5">
        <span className="text-xs text-[#6E675E]">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              currentPage === 1
                ? "cursor-not-allowed border-[#E8E4DE] text-[#716A61]"
                : "border-[#D9D4CE] bg-white text-[#625C54] hover:border-[#A69888] hover:text-[#675A4D]"
            )}
          >
            上一页
          </button>
          {getPageNumbers().map((page, idx) =>
            typeof page === "string" ? (
              <span
                key={`ellipsis-${idx}`}
                className="px-1.5 py-1.5 text-sm text-[#716A61]"
              >
                ...
              </span>
            ) : (
