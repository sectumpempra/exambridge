...
import { OCR_GCSE_BOUNDARIES } from "./ocrGradeBoundaries";
...
function buildIndex(records: VariantRecord[], meta: BoardMeta): Record<string, SubjectIndex> {
  ...
  // Strict release policy: collapse exact duplicates and quarantine ambiguous
  // conflicts instead of letting an arbitrary first row drive a prediction.
        ...
        if (variants.length > 1) {
          ...
          if (!hasIdentity || !uniqueIdentity) {
            ARCHIVED_BOUNDARY_CONFLICTS.push({
              ...
              reason: hasIdentity ? "duplicate-identity" : "missing-identity",
            });
...
export const DATA_INDEX: DataIndex = {
  caie: buildIndex(MERGED_CAIE_GCSE_DATA, BOARD_META["CAIE-GCSE"]),
  edexcel: buildIndex(edexcelJson as Record<string, string | number>[], BOARD_META["Edexcel-GCSE"]),
  ocr: buildIndex(OCR_GCSE_BOUNDARIES as unknown as Record<string, string | number>[], BOARD_META["OCR-GCSE"]),
  aqa: buildIndex(MERGED_AQA_GCSE_DATA, BOARD_META["AQA-GCSE"]),
  edexcel_al: buildIndex(edexcelALJson as Record<string, string | number>[], BOARD_META["Edexcel-AL"]),
  caie_al: buildIndex(MERGED_CAIE_A_LEVEL_DATA, BOARD_META["CAIE-AL"]),
  ocr_al: buildIndex(ocrALJson as Record<string, string | number>[], BOARD_META["OCR-AL"]),
  aqa_al: buildIndex(MERGED_AQA_A_LEVEL_DATA, BOARD_META["AQA-AL"]),
};

// Subject config as typed record
export const SUBJECTS_CONFIG = subjectsConfig as Record<string, SubjectConfig>;

/** Qualifications with complete, source-backed award routes enabled for prediction. */
export const VERIFIED_CALCULATOR_SUBJECTS: Readonly<Record<string, readonly string[]>> = {
  "Edexcel-AL": ["WMA"],
};

export function isVerifiedCalculatorSubject(boardKey: string, subjectCode: string): boolean {
  return VERIFIED_CALCULATOR_SUBJECTS[boardKey]?.includes(subjectCode) ?? false;
}
...
export function getSubjectsForBoard(boardKey: string): { code: string; name: string; category: SubjectCategory }[] {
  ...
  if (overrides) {
    if (boardKey === "Edexcel-AL") {
      ...
      return Array.from(grouped.values()).filter(subject =>
        isVerifiedCalculatorSubject(boardKey, subject.code)
      ).sort((a, b) => {
