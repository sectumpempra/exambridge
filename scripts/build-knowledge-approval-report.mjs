...
const approved = mappings.ownerApprovedCount === points.expectedCourseCount && mappings.candidateCount === 0;
...
const decision = approved ? "21 门课程已经所有者批准并进入 active" : ready ? "21 门课程已达到提交用户批准的条件" : "尚未达到提交用户批准的条件";
...
<section class="hero"><h1>ExamBridge 21 门知识映射批准报告</h1><p><strong>${decision}</strong></p><p class="muted">生成时间：${escapeHtml(new Date().toISOString())}。${approved ? "本报告记录本地 active 数据的所有者批准；尚未推送或部署。" : "本报告只针对本地修复分支的 candidate 数据；没有推送、部署或激活。"}</p>
...
<h2>批准结论</h2><section class="card"><p>当前状态：<span class="badge ${ready ? "pass" : "fail"}">${approved ? "OWNER APPROVED" : ready ? "READY FOR OWNER APPROVAL" : "NOT READY"}</span></p>
<p>${approved ? "所有 21 门映射均已写入批准日期与批准批次，并进入 active 数据层。" : ready ? "所有硬性条件已通过。用户仍需明确批准，系统才可以把 reviewStatus 从 candidate 改为 owner-approved。" : `仍需修正：${escapeHtml(unresolved.join("、") || "审计文件不完整")}。`}</p>
...
console.log(`Knowledge approval report written: ${approved ? "APPROVED" : ready ? "READY" : "NOT READY"}.`);
