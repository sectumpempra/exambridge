...
if (artifactManifest.universityAdmissionsManifestSha256 !== sha256(universityAdmissionsManifest)) throw new Error("AI university admissions manifest hash does not match the active data");
...
const aiSourceRoot = path.join(root, "server/ai");
const aiSourceFiles = (await readdir(aiSourceRoot, { recursive: true }))
  .filter(file => file.endsWith(".ts"));
for (const relativeFile of aiSourceFiles) {
  const source = await readFile(path.join(aiSourceRoot, relativeFile), "utf8");
  if (/(?:from\s+|import\s*\()\s*["'][^"']*data\/(?:candidates|quarantine)\/university-admissions-v1/i.test(source)) {
    throw new Error(`AI runtime source imports non-active university admissions data: ${relativeFile}`);
  }
}
