...
const policies = {
  "award:caie:0580": { series: ["march", "june", "november"], routes: ["award:caie:0580:core", "award:caie:0580:extended"], sourceUrls: [CAMBRIDGE_MARCH, CAMBRIDGE_RESULTS] },
  "award:caie:9709": { series: ["march", "june", "november"], routes: ["award:caie:9709:2023-2025:as:S1", "award:caie:9709:2023-2025:al:same-series:AX", "award:caie:9709:2023-2025:al:staged:DX"], sourceUrls: [CAMBRIDGE_MARCH, CAMBRIDGE_RESULTS] },
  "award:caie:9231": { series: ["march", "june", "november"], routes: ["award:caie:9231:as", "award:caie:9231:al"], sourceUrls: [CAMBRIDGE_MARCH, CAMBRIDGE_RESULTS] },
  "award:pearson:4ma1": { series: ["june", "november"], routes: ["award:pearson:4ma1:foundation", "award:pearson:4ma1:higher"], sourceUrls: [PEARSON_IGCSE, PEARSON_IGCSE_MANUAL] },
  "award:pearson:ial-mathematics": { series: ["january", "june", "october"], routes: ["award:pearson:ial-mathematics:YMA01"], sourceUrls: [PEARSON_IAL_MANUAL] },
  "award:pearson:ial-further-mathematics": { series: ["january", "june", "october"], routes: ["award:pearson:ial-further-mathematics:YFM01"], sourceUrls: [PEARSON_IAL_MANUAL] },
  "award:aqa:7357": { series: ["june"], routes: ["award:aqa:7357:linear"], sourceUrls: [AQA_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:aqa:7367": { series: ["june"], routes: ["award:aqa:7367:linear"], sourceUrls: [AQA_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:ocr:h240": { series: ["june"], routes: ["award:ocr:h240:linear"], sourceUrls: [OCR_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:ocr:h245": { series: ["june"], routes: ["award:ocr:h245:linear"], sourceUrls: [OCR_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:ocr:h640": { series: ["june"], routes: ["award:ocr:h640:linear"], sourceUrls: [OCR_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:ocr:6993": { series: ["june"], routes: ["award:ocr:6993:linear"], sourceUrls: [OCR_RESULTS, UK_CANCELLED_2020, UK_CANCELLED_2021] },
  "award:pearson:8ma0": { series: ["june"], routes: ["award:pearson:8ma0:linear"], sourceUrls: [UK_CANCELLED_2020, UK_CANCELLED_2021] },
};
...
export async function buildAcademicResultsCoverageMatrix(scope, candidate) {
  ...
  for (const qualification of scope.qualifications) {
    ...
    const observedSeries = new Set([
      ...candidate.boundaries.filter(row => row.awardQualificationId === qualification.awardQualificationId).map(row => row.series),
      ...candidate.statistics.filter(row => row.awardQualificationId === qualification.awardQualificationId).map(row => row.series),
    ]);
    const allSeries = [...new Set([...policy.series, ...observedSeries])];
    for (let year = scope.startYear; year <= scope.latestYear; year += 1) {
      for (const series of allSeries) {
        for (const routeId of policy.routes) {
          ...
          const notes = [];
          if (!policy.series.includes(series)) notes.push("Observed legacy series outside the current official recurring-series policy; requires historical confirmation.");
          ...
          cells.push({
            ...
            expectedByPolicy: policy.series.includes(series),
