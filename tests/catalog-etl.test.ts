...
describe("Catalog Query Interface", () => {
  ...
  it("indexes aliases case-insensitively and reports unknown kinds", () => {
    expect(catalogInstance.resolveAlias("board", "EDEXCEL")).toMatchObject({ kind: "FOUND", id: "board:pearson" });
    expect(catalogInstance.resolveAlias("qualification", "yma01")).toMatchObject({ kind: "FOUND", id: "qual:pearson:ial:yma01" });
    expect(catalogInstance.resolveAlias("unit", "p1").kind).toBe("FOUND");
    expect(catalogInstance.resolveAlias("sitting", "missing")).toEqual({ kind: "NOT_FOUND" });
  });

  it("covers every direct getter and empty list fallback", () => {
    const raw = catalogInstance.rawCatalog;
    expect(catalogInstance.getSpecification(raw.specifications[0].id)).toBeDefined();
    expect(catalogInstance.getPaper(raw.papers[0].id)).toBeDefined();
    expect(catalogInstance.getPaperVariant(raw.paperVariants[0].id)).toBeDefined();
    expect(catalogInstance.getBoundarySet(raw.boundarySets[0].id)).toBeDefined();
    expect(catalogInstance.getGradingScale(raw.gradingScales[0].id)).toBeDefined();
    expect(catalogInstance.getAggregationPolicy(raw.aggregationPolicies[0].id)).toBeDefined();
    expect(catalogInstance.getGradePolicy(raw.gradePolicies[0].id)).toBeDefined();
    expect(catalogInstance.getAStarPolicy(raw.aStarPolicies[0].id)).toBeDefined();
    expect(catalogInstance.getCalculationPolicy(raw.calculationPolicies[0].id)).toBeDefined();
    expect(catalogInstance.getSitting("missing")).toBeUndefined();
    expect(catalogInstance.listUnits("missing")).toEqual([]);
    expect(catalogInstance.listPapers("missing")).toEqual([]);
    expect(catalogInstance.listPaperVariants("missing")).toEqual([]);
    expect(catalogInstance.listRoutes("missing")).toEqual([]);
  });

  it("combines qualification filters and rejects malformed or out-of-range series", () => {
    expect(catalogInstance.listQualifications({ boardId: "board:pearson", level: "IAL", subjectCode: "YMA01" })).toHaveLength(1);
    expect(catalogInstance.listQualifications({ boardId: "missing" })).toEqual([]);
    expect(catalogInstance.getSpecificationForSeries("missing", "2025-june")).toBeUndefined();
    expect(catalogInstance.getSpecificationForSeries("qual:pearson:ial:yma01", "not-a-series")).toBeUndefined();
    expect(catalogInstance.getSpecificationForSeries("qual:pearson:ial:yma01", "1500-june")).toBeUndefined();
  });

  it("returns AMBIGUOUS for duplicate boundary identities", () => {
    const raw = structuredClone(catalogInstance.rawCatalog);
    raw.boundarySets.push({ ...raw.boundarySets[0], id: `${raw.boundarySets[0].id}:duplicate` });
    const duplicateCatalog = new Catalog(raw);
    const first = raw.boundarySets[0];
    expect(duplicateCatalog.getBoundary({ unitId: first.unitId, series: first.series }).kind).toBe("AMBIGUOUS");
  });
});
